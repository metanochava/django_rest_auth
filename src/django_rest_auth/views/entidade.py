...
from .serializers import (
    EntidadeSerializer,
    EntidadeGravarSerializer,
    EntidadeUserSerializer,
...
class EntidadeAPIView(viewsets.ModelViewSet):
    ...
    def addUser(self, request, *args, **kwargs):
        ...
        if not exists:
            ...
            return Response(
                {
                    "alert_seccess": f"O usuario {user.username} adicionado com sucesso!"
                },
                status=status.HTTP_201_CREATED
            )
        ...
        return Response(
            {
                "alert_seccess": f"O usuario {user.username} ja existe!"
            },
            status=status.HTTP_201_CREATED
        )
